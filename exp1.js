...
}) =>
{
  ...
  useEffect(() =>
  {
    ...
    const servicesContentEls = document.querySelectorAll('.testing .services-content');

    console.log(servicesContentEls);

    if (servicesContentEls.length > 0) {
      // Remove existing in case of re-render
      servicesContentEls.forEach(el =>
      {
        el.classList.remove('first-services-content', 'last-services-content');
      });

      // Add class to first
      servicesContentEls[0].classList.add('first-services-content');

      // Add class to last
      servicesContentEls[servicesContentEls.length - 1].classList.add('last-services-content');
    }
  }, []);
  return (
                  ...
                  <li key={index}>
                    {item.title && (
                      ...
                      <Typography
                        as="h4"
                        fontFamily="font-montserrat"
                        className={` ${item.icon ? "600px:text-[20px] flex flex-row justify-start items-center gap-2 768px:gap-2 text-[20px] mb-4" : "mb-5 768px:w-[80%] 600px:text-[26px] text-center text-[22px]"} leading-[30px] 600px:leading-[1.6em]  mx-auto font-bold text-[#000] 992px:text-left  992px:w-full ${index === 0 ? '768px:mt-2.5' : ' mt-7'
                          }`}
                      > {item.icon && (
                        <Image
                    ...
                    >
                      {item.details.map((paragraph, paragraphIndex) =>
                      {
                        return (
                          <div
                            key={paragraphIndex}
                            className={`text-justify 768px:text-start font-light leading-[2rem] 414px:pr-0 ${paragraphIndex === item.details.length - 1 ? 'mb-0' : 'mb-5'
                              } testing`}
                          >
