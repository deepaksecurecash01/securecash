...
const MobileMenu = ({ isVisible, activeSubMenu, onToggleSubmenu, onMenuClick }) => (
    <div id="mobile-menu" className={`block ${isVisible ? "h-screen" : ""} bg-white 1024px:hidden w-full`}>
        <button className="bg-primary w-full py-[15px] px-2.5 text-white pl-7" onClick={onMenuClick}>
            ...
            {MENU_ITEMS.map((item, index) => (
                ...
                >
                    {item.submenuId ? (
                        ...
                        <Link href={item.href} className="text-black ml-[20%]" onClick={onMenuClick}>
                            {item.name}
                        </Link>
                    )}
                </li>
...
const Navbar = () =>
{
    ...
    return (
        <>
            {/* Automatically shifts page content down */}
