...
const Navbar = () =>
{
  ...
  // --- Helper Components Defined Within Navbar Scope (for safe SSR) ---
  const Logo = ({ onClick }) => (
  ...
  const DesktopSubmenu = ({ links, onLinkClick }) => (
          ...
          (item, index) =>
          {
            ...
            return (
              <li key={index} className="leading-[50px] py-5 relative group">
                ...
                {hasSubMenu && <DesktopSubmenu links={item.links} onLinkClick={onLinkClick} />}
              </li>
            );
          }
        )}
  ...
  const MobileSubmenu = ({ links, isActive, onLinkClick }) => (
    <ul
  ...
  return (
    <div className="headroom-wrapper" role="banner" style={wrapperStyle}>
      ...
      <header
        ...
        // 💥 CRITICAL FIX: Use position: relative when not fixed to allow natural scroll-up behavior
