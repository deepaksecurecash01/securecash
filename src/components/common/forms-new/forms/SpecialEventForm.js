// /components/forms/SpecialEventForm.js
"use client";
...
// 2. STATIC IMPORT - Step 1
import SpecialEventBusinessStep from "./SpecialEvents/steps/SpecialEventBusinessStep.js";

// 3. DYNAMIC IMPORTS - Future Steps & Heavy Components
const SpecialEventContactStep = dynamic(() => import("./SpecialEvents/steps/SpecialEventContactStep.js"), {
  loading: () => <div className="h-64 w-full animate-pulse bg-white/5 rounded-md" />
});

const SpecialEventServiceStep = dynamic(() => import("./SpecialEvents/steps/SpecialEventServiceStep.js"), {
  loading: () => <div className="h-64 w-full animate-pulse bg-white/5 rounded-md" />
});

const SiteRiskFormFields = dynamic(() => import("./SiteRiskFormFields"), {
  loading: () => <div className="h-96 w-full animate-pulse bg-gray-100 rounded-md" />
});

const ThankYouModal = dynamic(() => import("./ThankYouModal"), {
  ssr: false
});
      ...
      <div id="content-contact" className="bg-content-bg bg-center bg-cover bg-no-repeat inline-block w-full 992px:my-[40px] 1280px:my-[120px]">
        <div className="inner-big w-[95%] max-w-[1366px] mx-auto my-0 992px:flex items-center">
          <div className="right-contact-row w-[96%] 992px:w-1/2 mx-auto 992px:mx-0 pt-[35px] 992px:pt-0 [flex:1] 992px:pl-8">
            <h3 className="text-[22px] 480px:mt-10 font-semibold leading-[1.6em] mx-auto 992px:text-[26px] 768px:text-left 768px:mx-0 font-montserrat">
            ...
            <h3 className="text-[22px] mt-10 mb-4 font-semibold leading-[1.6em] mx-auto 992px:text-[26px] 768px:text-left 768px:mx-0 font-montserrat">
              Please take note of the following conditions
            </h3>
            <p className="text-[16px] leading-[2rem] text-left mb-4 768px:text-left font-light font-montserrat">
              You MUST have your banking ready to be picked up prior to the arrival of the banking courier. The banking MUST be properly packaged in your nominated banks business express deposit satchels, if you need any clarification or help on how your banking needs to be prepared then please contact us as soon as possible and we will be more than happy to help.
            </p>
            <p className="text-[16px] leading-[2rem] text-left mb-4 768px:text-left font-light font-montserrat">
              Extra charges will apply at a rate of $95 plus GST per hour or part thereof if the banking courier is made to wait for banking that is not ready to be picked up upon their arrival at the time you booked. There will also be a charge of 0.75% of the total amount deposited plus GST if the banking is not properly prepared in your nominated banks business express deposit satchels, and the banking courier is unable to successfully lodge it on your behalf and needs to wait until a bank teller manually processes the deposit.
            </p>
            <p className="text-[16px] leading-[2rem] text-left mb-4 768px:text-left font-light font-montserrat">
              You <strong>must</strong> also inform your bank that you are having a banking courier service and advise them of the approximate amount of money you are expecting to be deposited, please don&apos;t assume that your deposit will be unconditionally accepted by the bank without them being notified accordingly. If a deposit is not accepted by the bank, then it will need to be returned back to your address at a rate of $275 plus GST.
            </p>
            <p className="text-[16px] leading-[2rem] text-left mb-4 768px:text-left font-light font-montserrat">
              If you are not after a once off collection but a regular collection fill out the form located{" "}
              <span className="underline"><strong className="uppercase"><a href="/site-info/">HERE</a></strong></span>{" "}
            ...
            <p className="text-[16px] leading-[2rem] text-left mb-4 768px:text-left font-light flex flex-col gap-4 font-montserrat">
              <span>To learn more about how we manage information provided you can view our{" "}<Link className="text-primary hover:underline" href="/privacy-policy/">Privacy Policy</Link>.</span>
              <strong><Link className="text-primary hover:underline" href="/austrac/">&lt;&lt; Previous</Link></strong>
            </p>
          </div>
              ...
              >
                {!isFirst && stepId !== "risk" && (
                  <div className="form-slide-btn-wrap mb-4 absolute">
                    <button type="button" onClick={formManager.goBack} className="flex items-center text-white hover:text-[#c6a54b] transition-colors">
                      <FaChevronLeft className="mr-2" /><span>Back</span>
                    </button>
                  </div>
                )}
                    ...
                    <div className="button-section relative">
                      ...
                      >
                        {formManager.isSubmitting ? (
                          <div className="flex items-center justify-center"><FaSpinner className="animate-spin mr-2" />Processing...</div>
      ...
      <div id="contact-form-section" className="inline-block w-full mb-12 480px:mb-[120px]">
        <div className="inner-big w-[95%] max-w-[1366px] mx-auto my-0 992px:flex">
          <div className="414px:mx-4 hidden 992px:block 992px:w-[50%] 992px:mx-0 py-8 px-10 480px:px-[5%] 992px:pl-8 1280px:pl-24 992px:pt-32 shadow-[3px_3px_10px_0px_rgba(0,0,0,0.2)] rounded-t-[8px] 992px:rounded-l-[8px] 992px:rounded-tr-none relative">
            <Image src="/images/welcome/terms-main-img-2.jpg" alt="Making A Deal" fill objectFit="cover" />
          </div>
          ...
          <div className={`float-none 992px:w-[80%] 992px:float-left relative left-0 flex justify-center transition-opacity duration-300 ${!submitButtonEnabled ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
            ...
            >
              <div className="form-tab 480px:w-[90%] mx-auto">
                <h3 className="text-[22px] font-semibold leading-[1.6em] mx-auto 992px:text-[26px] 768px:text-left 768px:mx-0 font-montserrat">
        ...
        {formManager.submissionError && (
          <div className="max-w-[1366px] mx-auto mt-4">
            <div className="text-red-600 text-center mb-4 p-4 bg-red-50 border border-red-200 rounded mx-4">
              ...
              <button onClick={() => window.location.reload()} className="ml-4 text-blue-600 hover:underline">Retry</button>
            </div>
          </div>
        )}
      </div>
