// /components/forms/SiteInfoForm.js
"use client";
...
// 2. STATIC IMPORT - Step 1 (Business)
// We keep this static so the form appears instantly without layout shift.
import SiteBusinessStep from "./SiteInfo/steps/SiteBusinessStep";

// 3. DYNAMIC IMPORTS - Future Steps & Heavy Components
// These load in the background, reducing the initial JS bundle size significantly.
const SiteContactStep = dynamic(() => import("./SiteInfo/steps/SiteContactStep"), {
  loading: () => <div className="h-64 w-full animate-pulse bg-white/5 rounded-md" />
});

const SiteServiceStep = dynamic(() => import("./SiteInfo/steps/SiteServiceStep"), {
  loading: () => <div className="h-64 w-full animate-pulse bg-white/5 rounded-md" />
});

const SiteRiskFormFields = dynamic(() => import("./SiteRiskFormFields"), {
  // This is below the fold, so a simple placeholder is fine
  loading: () => <div className="h-96 w-full animate-pulse bg-gray-100 rounded-md" />
});

const ThankYouModal = dynamic(() => import("./ThankYouModal"), {
  ssr: false // Modals are client-only
});
...
const SiteInfoForm = () =>
{
  ...
  const renderCurrentStep = () =>
  {
    const stepComponents = {
      business: SiteBusinessStep, // Rendered statically
      contact: SiteContactStep,   // Rendered dynamically
      service: SiteServiceStep,   // Rendered dynamically
      risk: () => (
        ...
        <div className="inner-big w-[95%] max-w-[1366px] mx-auto my-0 992px:flex items-center">
          <div className="right-contact-row w-[96%] 992px:w-1/2 mx-auto 992px:mx-0 pt-[35px] 992px:pt-0 [flex:1] 992px:pl-8">
            ...
            <p className="text-[16px] leading-[2rem] text-left mb-4 768px:text-left font-light font-montserrat">
              ...
              <span className="underline"><strong className="uppercase"><a href="/special-event/">HERE</a></strong></span>{" "}
              instead.
            </p>
          ...
          <div className={`float-none 992px:w-[80%] 992px:float-left relative left-0 flex justify-center transition-opacity duration-300 ${!submitButtonEnabled ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
            ...
            >
              <div className="form-tab 480px:w-[90%] mx-auto">
                ...
                {/* 🚀 DYNAMIC COMPONENT - Heavily reduces initial bundle */}
